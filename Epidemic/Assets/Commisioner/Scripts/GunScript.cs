...
public class GunScript : MonoBehaviour {
	...
	*/
	...
	public TextMesh Commisioner_blood;
	public int commisioner_blood_left;
	void OnGUI(){
		if(!HUD_bullets){
			try{
        ...
        if (!Commisioner_blood)
		{
            try
            {
				Commisioner_blood = GameObject.Find("Commisioner_blood").GetComponent<TextMesh>();
				
            }
			catch (System.Exception ex)
			{
				print("Couldnt find the Commisioner_blood ->" + ex.StackTrace.ToString());
			}
		}
		if(Commisioner_blood)
        {
			Health cm_blood = new Health();
			//cm_blood.LoseBlood(10);
			Commisioner_blood.text = "blood left: " + cm_blood.currentHealth.ToString();
		}
