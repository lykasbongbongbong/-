...
public enum Infector_GunStyles
{
	nonautomatic,automatic
}
public class Infector_GunScript : MonoBehaviour {
	[Tooltip("Selects type of waepon to shoot rapidly or one bullet per click.")]
	public Infector_GunStyles currentStyle;
	[HideInInspector]
	public Infector_MouseLookScript mls;
	...
	public float amountOfBulletsPerLoad = 10;

	private Transform player;
	private Camera cameraComponent;
	private Transform gunPlaceHolder;

	private Infector_PlayerMovementScript pmS;
	...
	void Awake(){
		...
		mls = GameObject.FindGameObjectWithTag("Infector").GetComponent<Infector_MouseLookScript>();
		...
		pmS = player.GetComponent<Infector_PlayerMovementScript>();
	...
	*/
	...
	void CrossHairExpansionWhenWalking(){
		...
		if(player.GetComponent<Rigidbody>().velocity.magnitude > 1 && Input.GetAxis("Fire1") == 0){//ifnot shooting
			...
			if(player.GetComponent<Infector_PlayerMovementScript>().maxSpeed < runningSpeed){ //not running
	...
	*/
	...
	void Shooting(){
		...
		if (!meeleAttack) {
			if (currentStyle == Infector_GunStyles.nonautomatic) {
				if (Input.GetButtonDown ("Fire1")) {
					ShootMethod ();
				}
			}
			if (currentStyle == Infector_GunStyles.automatic) {
	...
	*/
	...
	IEnumerator Reload_Animation(){  
		if(bulletsIHave > 0 && bulletsInTheGun < amountOfBulletsPerLoad && !reloading/* && !aiming*/){
			...
			if (meeleAttack == false && pmS.maxSpeed != runningSpeed) {
				//print ("tu sam");
				if (player.GetComponent<Infector_PlayerMovementScript> ()._freakingZombiesSound)
					player.GetComponent<Infector_PlayerMovementScript> ()._freakingZombiesSound.Play ();
				else
	...
	public TextMesh Infector_blood;
	
	void OnGUI(){
		if(!HUD_bullets){
			try{
				HUD_bullets = GameObject.Find("Infector_left_bullet").GetComponent<TextMesh>();
			}
			catch(System.Exception ex){
				print("Couldnt find the Infector_left_bullet ->" + ex.StackTrace.ToString());
			}
		}
		if(mls && HUD_bullets)
        {
			HUD_bullets.text = "剩餘子彈：" + bulletsIHave.ToString() + " - " + bulletsInTheGun.ToString();
		}
        ...
        if (!Infector_blood)
		{
            try
            {
				Infector_blood = GameObject.Find("Infector_blood").GetComponent<TextMesh>();
				
            }
			catch (System.Exception ex)
			{
				print("Couldnt find the Infector_blood ->" + ex.StackTrace.ToString());
			}
		}
		if(Infector_blood)
        {
			Infector_Health cm_blood = new Infector_Health();
			//cm_blood.LoseBlood(10);
			Infector_blood.text = "剩餘血量: " + cm_blood.currentHealth.ToString();
		}
