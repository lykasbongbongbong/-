...
public enum Infector_MenuStyle
{
	horizontal,vertical
}

public class Infector_GunInventory : MonoBehaviour {
	...
	*/
	void AssignHandsAnimator(GameObject _currentGun){
		if(_currentGun.name.Contains("Gun")){
			currentHAndsAnimator = currentGun.GetComponent<Infector_GunScript>().handsAnimator;
		}
	}
	...
	public Infector_MenuStyle menuStyle = Infector_MenuStyle.horizontal;
	...
	void DrawCorrespondingImage(int _number){
		...
		if(menuStyle == Infector_MenuStyle.horizontal){
		...
		else if(menuStyle == Infector_MenuStyle.vertical){
			if(deleteCloneFromName == gunsIHave[_number]){
