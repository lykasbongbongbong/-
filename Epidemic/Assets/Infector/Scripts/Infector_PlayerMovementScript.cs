...
public class Infector_PlayerMovementScript : MonoBehaviour {
	Rigidbody rb;
	...
	*/
	...
	private void RaycastForMeleeAttacks(){
		...
		if (GetComponent<Infector_GunInventory> ().currentGun) {
			if (GetComponent<Infector_GunInventory> ().currentGun.GetComponent<Infector_GunScript> ()) 
				currentWeapo = "gun";
		}
		...
		if (GetComponent<Infector_GunInventory> ().currentGun) {
			if (GetComponent<Infector_GunInventory> ().currentGun.GetComponent<Infector_GunScript> ().meeleAttack == false) {
				been_to_meele_anim = false;
			}
			if (GetComponent<Infector_GunInventory> ().currentGun.GetComponent<Infector_GunScript> ().meeleAttack == true && been_to_meele_anim == false) {
	...
	*/
	void InstantiateBlood (RaycastHit _hitPos,bool swordHitWithGunOrNot) {		
		...
		if (currentWeapo == "gun") {
			Infector_GunScript.HitMarkerSound ();
