...
public class Infector_MouseLookScript : MonoBehaviour {
	...
	*/
	void  Update(){
		...
		if(GetComponent<Infector_PlayerMovementScript>().currentSpeed > 1)
			HeadMovement ();
...
void FixedUpdate(){
	...
	else if(GetComponent<Infector_PlayerMovementScript>().maxSpeed > 5){
...
private Infector_GunScript gun;
/*
...
void WeaponRotation(){
	if(!weapon){
		...
		if(weapon){
			if(weapon.GetComponent<Infector_GunScript>()){
				try{
					gun = GameObject.FindGameObjectWithTag("Weapon").GetComponent<Infector_GunScript>();
